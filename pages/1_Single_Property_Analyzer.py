...
if zori_rent:
    ...
    rent_max = int(zori_adjusted * 2.00)
    st.sidebar.markdown("**Monthly Rent ($)**")
    st.sidebar.caption(f"Zillow ZORI: ${zori_rent:,}/mo × 1.3 = ${zori_adjusted:,}/mo (Bay Area adjusted) — ±100% from adjusted default")
...
with tab2:
    ...
    fig_exp, ax_exp = plt.subplots(figsize=(4, 4))
    ...
    legend_labels = [
        f"Op. Expenses: {value_labels[0]}",
        f"Mortgage: {value_labels[1]}",
        f"Cash Flow: {value_labels[2]}"
    ]
    ax_exp.legend(wedges, legend_labels, loc="lower center",
        bbox_to_anchor=(0.5, -0.22), frameon=False, ncol=1, fontsize=9)
    ax_exp.axis("equal")
